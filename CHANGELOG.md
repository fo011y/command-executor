...
### Dashboard — редизайн и светлая тема

- `Dashboard.jsx` — полный редизайн: страница разделена на две части (верхняя — команды, нижняя — GPS-заглушка)
- Кнопки команд переработаны в стиле иконок приложений: квадратные 100×100px, SVG-иконка по смыслу команды (запуск, стоп, открыть, закрыть, поднять, опустить и др.)
- Группировка команд по бренду (Ford/Volvo) через таб-переключатель, внутри — по подкатегориям
- GPS-блок: стилизованная заглушка с сеткой, угловыми маркерами, пульсирующим перекрестием и меткой «GPS ТРЕКИНГ»
- Добавлен переключатель тёмной/светлой темы с сохранением выбора в `localStorage` (`gcb-theme`)
- `Dashboard.css` — полностью переписан: CSS-переменные для двух тем (`[data-theme="dark"]` / `[data-theme="light"]`), применяется через `data-theme` на `<html>`

### Профиль — убран блок команд

- `Profile.jsx` — удалён блок «Доступные команды»; команды отображаются только на Dashboard
- Удалены состояния `commandGroups`, `cmdLoading`, обработчик `handleExecuteCommand` и весь связанный рендер
